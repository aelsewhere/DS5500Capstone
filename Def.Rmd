  ...
  for (i in 1:nrow(nfl_data)) {
    ...
    for (j in seq_along(defensive_player_ids)) {
      ...
      if (!is.na(defensive_player_ids[j])) {
        points <- points + 4  # Assuming 4 points per interception
      }

      # Check for fumble recovery
      if (!is.na(defensive_player_ids[j])) {
        points <- points + 2  # Assuming 2 points per fumble recovery
      }

      # Check for sack
      if (!is.na(play$sack) && play$sack > 0) {
        points <- points + (2 * play$sack)  # Assuming 2 points per sack
      }

      # Check for safety
      if (!is.na(play$safety) && play$safety > 0) {
        points <- points + (2 * play$safety)  # Assuming 2 points per safety
      }

      # Check for tackle for loss
      if (!is.na(play$tackled_for_loss) && play$tackled_for_loss > 0) {
        points <- points + play$tackled_for_loss  # Assuming 1 point per tackle for loss
      }

      # Check for solo tackle
      if (!is.na(play$solo_tackle) && play$solo_tackle > 0) {
        points <- points + play$solo_tackle  # Assuming 1 point per solo tackle
      }

      # Check for blocked punt
      if (!is.na(play$punt_blocked) && play$punt_blocked > 0) {
        points <- points + 2  # Assuming 2 points per blocked punt
      }
...
# Create a histogram of total points
ggplot(player_totals, aes(x = total_points)) +
  geom_histogram(binwidth = 5, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Total Fantasy Points Distribution by Player",
...
# Assuming the column to be removed is "column_name"
...
# Assuming your training data is named 'train_data'
...
elastic_net_model <- cv.glmnet(as.matrix(train_data_scaled), y_train, alpha = 0.5)
...
column_names <- names(train_data_scaled_enr)
print(column_names)
```
...
coef_values <- coef(enet_model, s = best_lambda)
...
# Order the data frame by absolute coefficient values in descending order
